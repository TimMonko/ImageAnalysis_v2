...
re.file.data.wide <- reactive({
  re.file.data() %>%
    pivot_wider(id_cols = c(file, genotype, pair, region), names_from = z, values_from = n.cells)
})
...
THIS IS WHERE I'M STUCK, probably just hardcode with oldschool mutates and just get it to check like 5 groups and that's plenty, I guess? need to get a reactive variable name but it's probably just !!paste(input$numerator)

```{r relative_slice_data, echo=FALSE}
wellPanel(
  selectizeInput("numerator", label = "numerator slice", choices = unique(all.data$z), multiple = TRUE),
  selectizeInput("denominator", label = "denominator slice", choices = unique(all.data$z), multiple = TRUE)
)
...
re.divide.data <- reactive({
  re.pair.data() %>%
    group_by(pair, region, genotype) %>%
    summarize(rel1 = n.cells.mean[z == as.numeric(input$numerator[1])]/n.cells.mean[z == as.numeric(input$denominator[1])],
              rel2 = n.cells.mean[z == as.numeric(input$numerator[2])]/n.cells.mean[z == as.numeric(input$denominator[2])],
              rel3 = n.cells.mean[z == as.numeric(input$numerator[3])]/n.cells.mean[z == as.numeric(input$denominator[3])],
              rel4 = n.cells.mean[z == as.numeric(input$numerator[4])]/n.cells.mean[z == as.numeric(input$denominator[4])],
              rel5 = n.cells.mean[z == as.numeric(input$numerator[5])]/n.cells.mean[z == as.numeric(input$denominator[5])]) %>%
    distinct()
})


renderDT(re.divide.data(), filter="top", options = list(pageLength = 5, scrollX=TRUE))
```
